...
import time
import actionlib
roslib.load_manifest("open_industrial_web_interface")

from open_industrial_web_interface.msg import MoveGroupPlan
...
#        start_state.joint_state.header.stamp = rospy.Time.now()
        ...
        cons.name = ""
        i = 0
        for dim in joints.joint.layout.dim:
            start_state.joint_state.name.append(dim.label)
            if dim.label == "r_elbow_flex_joint": #tmp
                start_state.joint_state.position.append(-0.1)
            elif dim.label == "l_elbow_flex_joint": #tmp
                start_state.joint_state.position.append(-0.1)
            else:
                start_state.joint_state.position.append(joint_states[dim.label])
            jc = JointConstraint()
            jc.joint_name = dim.label
            jc.position = joints.joint.data[i]
